...
async function main() {
  try {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [...marketDataTools, ...indicatorTools, ...accountTools, ...orderTools, ...swapTools, ...flashLoanTools],
      };
    });

    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      const handler = { ...marketDataHandlers, ...indicatorHandlers, ...accountHandlers, ...orderHandlers, ...swapHandlers, ...flashLoanHandlers }[request.params.name];
      if (!handler) {
    ...
    process.stderr.write(`[deepbook-mcp] Tools registered: ${marketDataTools.length + indicatorTools.length + accountTools.length + orderTools.length + swapTools.length + flashLoanTools.length}\n`);
  } catch (error) {
